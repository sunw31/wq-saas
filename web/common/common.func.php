...
function checklogin($url = '') {
	global $_W;
	if (empty($_W['uid'])) {
		if (!empty($_W['isw7_sign']) || !empty($_SERVER['HTTP_SEC_FETCH_DEST']) && 'iframe' == $_SERVER['HTTP_SEC_FETCH_DEST']) {
	...
	if (empty($cookie['rember']) && empty($_W['isw7_sign'])) {
...
function buildframes($framename = '') {
	...
	if ((!empty($modulename) || !empty($eid)) && !in_array($modulename, module_system())) {
		...
		$account_user_role = permission_account_user_role($_W['uid'], $_W['uniacid']); 
...
function check_w7_request($action = '') {
	global $_W;
	...
	$urls = array(
		'sms-sign' => '/system/sms/sign', 
		'sms-package' => '/system/sms/package', 
		'sms-statistics' => '/system/sms/statistics', 
		'sms-template' => '/system/sms/template', 
		'sms-share' => '/system/sms/share', 
		'account' => '/system/statistics/module', 
		'profile' => '/basicInfo', 
		'diagnose' => '/site/diagnose', 
		'site' => '/site/setting/basic', 
		'menu' => '/site/setting/menu', 
		'attachment' => '/site/attachment/global', 
		'systeminfo' => '/system/info', 
		'logs' => '/site/logs/wechat', 
		'ipwhitelist' => '/site/filter/ipwhitelist', 
		'sensitiveword' => '/site/filter/sensitiveword', 
		'registerset' => '/site/registerset/display', 
		'oauth' => '/site/oauth/display', 
		'filecheck' => '/system/check/file', 
		'optimize' => '/system/check/optimize', 
		'database' => '/system/database/backup', 
		'scan' => '/system/scan/horse', 
		'bom' => '/system/check/bom', 
		'check' => '/system/check/sys', 
		'job' => '/site/backtask', 
	);
	$notice = '最新版系统功能已经迁移到控制台，请前往控制台操作。';
	$console_siteurl = 'http://console.w7.cc/console/' . $_W['setting']['site']['key'] . '/we7';
	$redirect = empty($urls[$action]) ? $console_siteurl : ($console_siteurl . $urls[$action]);
