...
if ('save_account' == $do) {
	...
	if ($_W['isfounder']) {
		...
		if (!empty($_GPC['groups'])) {
			...
			$group_modules = uni_groups($_GPC['groups']);
			$module_name = array_reduce(array_column($group_modules, 'modules_all'), 'array_merge', array());
	...
	if (in_array($sign, array(PHONEAPP_TYPE_SIGN, WXAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) {
		...
		if (!empty($module)) {
			$version_modules = array($module['name']);
			if (!empty($module_expired_list)) {
				$expired_modules_name = module_expired_diff($module_expired_list, $version_modules);
				if (!empty($expired_modules_name)) {
					iajax('-1', '应用：' . $expired_modules_name . '，服务费到期，无法添加！');
				}
			}
