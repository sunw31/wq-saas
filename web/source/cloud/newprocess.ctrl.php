...
if (in_array($do, array('files', 'module_install', 'module_build')) && empty($_GPC['code']) && !empty($_GPC['module_name'])) {
	$module_cloud = pdo_get('modules_cloud', array('name' => safe_gpc_string($_GPC['module_name'])), array('cloud_id', 'name', 'version'));
	if (!empty($module_cloud) && -1 == $module_cloud['cloud_id']) {
		$code = cloud_m_code_generate(array('module_name' => $module_cloud['name'], 'module_version' => $module_cloud['version']));
		if (is_error($code)) {
			iajax(-1, $code);
		}
	}
...
if ('module_install' == $do) {
	...
	$module['status'] = ((empty($module_info['status']) || empty($module_info['branches'][$module_info['branch_id']]['status'])) && empty($code)) ? STATUS_OFF : STATUS_ON;
