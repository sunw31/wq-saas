...
if ('check' == $do) {
	$filetree = file_tree(IA_ROOT, array('api', 'app', 'framework', 'payment', 'web', 'api.php', 'index.php', 'data'));
	...
	if (!empty($cloud_data['files'])) {
		...
		foreach ($filetree as $filename) {
			...
			$ignore_list = array(
					...
					0 === strpos($file, '/data/update'),
					0 === strpos($file, '/data/patch'),
					0 === strpos($file, '/data/module'),
