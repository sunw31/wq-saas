...
function file_remote_upload($filename, $auto_delete_local = true) {
	...
	if ($_W['setting']['remote']['type'] == ATTACH_FTP) {
		...
		$upload_link = ihttp_request('http://api.weixin.qq.com/tcb/uploadfile', json_encode(array('env' => getenv('CBR_ENV_ID'), 'path' => $filename)));
		$upload_link = json_decode($upload_link['content'], true);
		if (0 != $upload_link['errcode']) {
			return error(1, '上传云托管时发生错误，错误码：' . $upload_link['errocde'] . '，详情：' . $upload_link['errmsg']);
		}
...
function file_remote_delete($file) {
	...
	switch ($_W['setting']['remote']['type']) {
		...
		case ATTACH_TCB:
			global $media;
			$api_delete = ihttp_request('http://api.weixin.qq.com/tcb/batchdeletefile', json_encode(array('env' => getenv('CBR_ENV_ID'), 'fileid_list' => array($media['tcb_file_id']))));
			$api_delete = json_decode($api_delete['content'], true);
			if (0 != $api_delete['errcode']) {
				return error(1, '删除失败，错误码：' . $api_delete['errocde'] . '，详情：' . $api_delete['errmsg']);
			}
			break;
	}
	return $result;
