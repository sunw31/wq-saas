		...
		if (!empty($recycle_info)) {
			...
			foreach (module_support_type() as $support => $value) {
				if (!empty($recycle_info[MODULE_RECYCLE_UNINSTALL_IGNORE][$support])) {
					$module_info['recycle_info'][$support] = MODULE_RECYCLE_UNINSTALL_IGNORE; 
				} else {
...
function module_upgrade_info($modulelist = array()) {
	...
	$cloud_module_code_query = cloud_m_code_query();
	if (is_error($cloud_module_code_query)) {
		return $cloud_module_code_query;
	}
	$cloud_module_code_query = array_column($cloud_module_code_query, null, 'name');
	...
	foreach ($modulelist as $modulename => $module) {
		if (!empty($cloud_module_code_query[$modulename])) {
			unset($cloud_module_code_query[$modulename]);
		}
		...
		$module_upgrade_data['modified_time'] = $manifest_cloud_list[$modulename]['site_branch']['modified_time'];
		...
		$module_upgrade_data['service_expire_time'] = $manifest_cloud_list[$modulename]['service_expiretime'];
		$module_upgrade_data['system_shutdown_time'] = $manifest_cloud_list[$modulename]['system_shutdown_delay_time'];
		$module_upgrade_data['status'] = $status;
		$module_upgrade_data['cloud_id'] = $manifest_cloud_list[$modulename]['cloud_id'];
		...
		if (STATUS_ON == $result[$modulename]['new_version']) {
			$result[$modulename]['new_version'] = empty($status) || TIMESTAMP > $manifest['service_expiretime'] || !empty($manifest['system_shutdown']) && 1 < $manifest['system_shutdown'] ? STATUS_OFF : STATUS_ON;
		}
	...
	if (!empty($manifest_cloud_list)) {
		foreach ($manifest_cloud_list as $modulename => $manifest) {
			$module_upgrade_data = array(
				...
				'modified_time' => $manifest['site_branch']['modified_time'],
				'buytime' => $manifest['buytime'],
				'cloud_id' => $manifest['cloud_id'],
				'label' => $manifest['label'],
				'main_module_name' => $manifest['main_module'],
				'service_expire_time' => $manifest['service_expiretime'],
				'system_shutdown_time' => $manifest['system_shutdown_delay_time'],
				'status' => empty($manifest['status']) || empty($manifest['site_branch']['status']) || ($manifest['system_shutdown_delay_time'] && TIMESTAMP > $manifest['system_shutdown_delay_time']) ? STATUS_OFF : STATUS_ON,
			);
			if (!empty($manifest['platform']['supports'])) {
				foreach ($module_support_type as $support_key => $support_value) {
					if (in_array($support_value['type'], $manifest['platform']['supports'])) {
	...
	if (!empty($cloud_module_code_query)) {
		foreach ($cloud_module_code_query as $item) {
			$module_cloud_upgrade = table('modules_cloud')->getByName($item['name']);
			$module_upgrade_data = array(
				'name' => $item['name'],
				'title' => $item['title'],
				'logo' => $item['logo'],
				...
				'install_status' => MODULE_CLOUD_UNINSTALL,
				'application_type' => 1,
				'title_initial' => get_first_pinyin($item['title']),
				'status' => STATUS_ON,
			...
			if (empty($module_cloud_upgrade)) {
				...
				pdo_update('modules_cloud', $module_upgrade_data, array('name' => $item['name']));
			}
		}
	}
	return $result;
...
function module_name_list($group_id = array()) {
	...
	$group_module = pdo_getall('uni_group', array('id IN ' => $group_id), array('modules'));
...
function module_permission_check($message_type = '') {
	global $_W,$controller,$action,$do;
	$api_whitelist = array(
		'c=cloud&a=newprocess&do=module_build', 
		'c=module&a=group&do=save', 
		'c=utility&a=group&do=get_module_group_detail_info', 
		'c=module&a=group&do=del', 
		'c=account&a=post&do=', 
		'c=account&a=post&do=modules_tpl', 
		'c=account&a=post-user&do=edit', 
		'c=account&a=post-user&do=set_manager', 
		'c=miniapp&a=manage&do=display', 
		'c=utility&a=modules&do=modules', 
		'c=user&a=edit&do=', 
		'c=user&a=create&do=check_user_info', 
		'c=utility&a=group&do=get_user_group_detail_info', 
		'c=user&a=group&do=save', 
	);
	$cado = 'c=' . $controller . '&a=' . $action . '&do=' . $do;
	if (!empty($_W['config']['setting']['local_dev']) || !in_array($cado, $api_whitelist)) {
		return true;
	}
	$cloud_modules_cachekey = cache_system_key('cloud_expired_modules');
	$cloud_module_cache = cache_load($cloud_modules_cachekey);
	if (empty($cloud_module_cache)) {
		$cloud_module_cache = module_cloud_expired_list();
	}
	$local_modules_cachekey = cache_system_key('local_dev_modules');
	$local_module_cache = cache_load($local_modules_cachekey);
	if (empty($local_module_cache)) {
		...
		cache_write($local_modules_cachekey, $local_module_cache);
	}
	
	if (!empty($_W['uid']) && ($local_module_cache || $cloud_module_cache)) {
		$message = empty($_W['isw7_request']) ? '运营模式下请联系管理员卸载本地应用！' : '打开开发模式进入应用管理-需卸载应用列表，卸载完毕后关闭开发模式，恢复正常使用！';
		if (!empty($cloud_module_cache)) {
			$message = $cloud_module_cache . ' 应用测试码已过期，请尽快卸载！';
		}
		switch ($message_type) {
			case 'message':
				message($message);
...
function module_cloud_expired_list() {
	$cachekey = cache_system_key('cloud_expired_modules');
	$cloud_module_cache = cache_load($cachekey);
	if (!empty($cloud_module_cache)) {
		return $cloud_module_cache;
	}
	$cloud_expired_modules = '';
	$modules = pdo_getall('modules', array('from' => 'cloud_test'), array('code'));
	if (!empty($modules)) {
		$codes = array_column($modules, 'code');
		$result = cloud_m_code_available($codes);
		if (is_error($result)) {
			return $result;
		}
		$available_codes = empty($result) ? array() : array_keys($result);
		$module_codes = array_diff($codes, $available_codes);
		if (!empty($module_codes)) {
			$module_titles = pdo_getall('modules', array('from' => 'cloud_test', 'code IN' => $module_codes), array('title'));
			$cloud_expired_modules = implode('，', array_column($module_titles, 'title'));
		}
	}
	cache_write($cachekey, $cloud_expired_modules);
	return $cloud_expired_modules;
}
