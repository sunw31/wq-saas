...
function refund($refund_id) {
	...
	if ($paylog['type'] == 'wechat' || $paylog['type'] == 'wxapp') {
		...
		$cert_file = ATTACHMENT_ROOT . (($refundlog['is_wish'] == 1) ? 'store' : $_W['uniacid']) . md5(complex_authkey()) . '_wechat_refund_all.pem';
...
function reufnd_wechat_build($refund_id, $is_wish = 0) {
	...
	$cert_file = ATTACHMENT_ROOT . ($is_wish == 1 ? 'store' : $_W['uniacid']) . md5(complex_authkey()) . '_wechat_refund_all.pem';
