...
function user_single($user_or_uid) {
	...
	if (!empty($third_info) && is_array($third_info)) {
		...
		$record['console'] = empty($third_info[USER_REGISTER_TYPE_CONSOLE]) ? STATUS_OFF : STATUS_ON;
	}
...
function user_is_bind() {
	global $_W;
	if (!empty($_W['isw7_request']) || $_W['isadmin']) {
		return true;
	}
	...
	if (empty($setting_bind)) {
		$setting_bind = $_W['setting']['copyright']['bind'] = 'console';
	}
