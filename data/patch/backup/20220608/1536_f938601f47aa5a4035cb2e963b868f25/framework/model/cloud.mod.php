...
function cloud_m_code_query() {
	return cloud_api('module/code/query');
}
...
function cloud_unbind_user($uid) {
	return cloud_api('we7/oauth/user-bind/unbind', array('out_user_id' => $uid));
}
...
function cloud_verify_user_by_token($token) {
	global $_W;
	if (empty($token) || empty($_W['setting']['site']) || empty($_W['setting']['site']['key'])) {
		return error(-1, '获取站点信息失败！');
	}
	$cache_key = cache_system_key('cloud_w7_request_user_token', array('token' => md5($token)));
	$cache = cache_load($cache_key);
	if (!empty($cache)) {
		return $cache;
	}
	$ret = cloud_api('site/oauth/user/web-token/verify', array('token' => $token));
	if (is_error($ret)) {
		return $ret;
	}
	if ($ret['site_key'] != $_W['setting']['site']['key']) {
		return error(-1, '站点信息异常！');
	}
	if (empty($ret['out_user_id'])) {
		return error(-1, '请先登录客户端绑定控制台账号！');
	}
	$out_user_id = intval($ret['out_user_id']);
	cache_write($cache_key, $out_user_id, $ret['expire_time']);
	return $out_user_id;
}
...
function cloud_oauth_user($token) {
	return cloud_api('site/oauth/user/info', array('access_token' => $token));
}
