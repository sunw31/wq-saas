...
$console_module_page_white_list = array(
	array('account', 'auth', $do),
	array('site', 'entry', ''),
	array('user', 'logout', ''),
	array('home', 'welcome', 'ext'),
	array('module', 'welcome', 'welcome_display'),
);
if (empty($_W['isadmin']) && 'document' == $_SERVER['HTTP_SEC_FETCH_DEST'] && !in_array(array($controller, $action, $do), $console_module_page_white_list)) {
...
unset($console_module_page_white_list);
...
if (((int)ENDTIME - (int)STARTTIME) > $_W['config']['setting']['maxtimeurl']) {
	$data = array(
		'type' => '1',
