...
function template($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$console_white_list = array(
		'user/login-base',
		'user/login-big',
		'user/login-half',
		'user/login-half-auto',
		'user/console',
		'common/message',
		'utility/user-browser'
	);
	if (!empty($_SERVER['HTTP_SEC_FETCH_DEST']) && 'document' == $_SERVER['HTTP_SEC_FETCH_DEST'] && $_W['isw7_sign'] && !in_array($filename, $console_white_list) && $flag == TEMPLATE_DISPLAY) {
		$filename = 'user/console';
	}
